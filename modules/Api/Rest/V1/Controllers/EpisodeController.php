...
class EpisodeController extends Controller
{
    ...
    public function attemptCreate(): ResponseInterface
    {
        ...
        if (! $this->validate($rules)) {
            return $this->failValidationErrors(current($this->validator->getErrors()));
        }
        ...
        $updatedByUserModel = new UserModel();
        $updatedByUser = $updatedByUserModel->find($updatedByUserId);
        ...
        if ((new EpisodeModel())
            ...
            return $this->fail('An episode with the same slug already exists in this podcast.', 400);
        }
        ...
        $newEpisode = new Episode([
            ...
            'description_markdown' => in_array($this->request->getPost('description'), ['', null], true)
                ? ''
    ...
    public function attemptPublish(int $id): ResponseInterface
    {
        ...
        $podcastModel = new PodcastModel();
        $podcast = $podcastModel->find($episode->podcast_id);
        ...
        if ($episode->publication_status !== 'not_published') {
            return $this->fail('Episode is already published or scheduled for publication', 400);
        }
        ...
        $clientTimezone = $this->request->getPost('client_timezone') ?? 'UTC';

        if ($podcast->publication_status === 'published') {
        ...
        } elseif ($podcast->publication_status === 'scheduled') {
